...
export function CatPage(props) {
  ...
  function renderCats(item) {
      ...
      <TouchableOpacity
        onPress={() => {
          ...
          setOffersdata(spiisficItems(item.item));
        ...
        style={{
          ...
          flex:1,
          backgroundColor:
            item.item._id == pressed2.id ||
            item.item.titleAR == pressed2.titleAR ||
            item.item.titleEN == pressed2.titleEN
              ? '#B70B0B'
              : null,
        }}>
        <Text
          ...
          {i18n.language == 'ar' ? item.item.titleAR : item.item.titleEN}
        </Text>
  ...
  const [list, setList] = useState([
    {titleAR: 'الكل', titleEN: 'All', _id: 'edq23ee'},
  ]);
  ...
  return (
    <>
    {loading && <ActivityIndicator  animating={true} style={{width:'100%',height:'100%'}} size={60} color="black" />}
    <View style={{flex: 1}}>
      {deatils(pressed)}
      {MyHeader(
      ...
      <View style={styles.main}>
        <TextInput
          ...
          onChangeText={(text)=>{ 
           setOffersdata(selectedCatOffer.filter((item,index)=>{
              if(text==''){
                return true
              }
              let re = new RegExp(`${text}`, "gi");
              return re.test(item.company.fullnameEN)||re.test(item.company.fullnameAR)
